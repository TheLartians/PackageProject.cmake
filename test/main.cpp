...
int main() {
  ...
  result &= DEPENDENCY_VERSION_PATCH == 3;
  result &= DEPENDENCY_VERSION_BUILD == std::string("0");
  ...
  result &= NAMESPACED_DEPENDENCY_VERSION_BUILD == std::string("0");
  ...
  result &= TRANSITIVE_DEPENDENCY_VERSION_BUILD == std::string("21948124");
  return result ? 0 : 1;
}
